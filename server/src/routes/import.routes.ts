...
import { sendImportSummaryEmail } from '../services/email.service';
...
function autoSuggestMapping(headers: string[]): Record<string, string> {
  ...
  for (const [field, aliases] of Object.entries(FIELD_ALIASES)) {
    const matched = normalizedHeaders.find((h) =>
      aliases.some((alias) => h.cleaned === alias || h.cleaned.includes(alias))
    );
    if (matched) {
...
router.post('/validate', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const mappedRows = rows.map((row: any, index: number) => {
      ...
      if (rawEmail) candidateEmails.push(rawEmail);
      if (rawPhone) candidatePhones.push(rawPhone);
...
router.post('/commit', async (req: AuthRequest, res: Response): Promise<void> => {
  try {
      ...
      try {
        ...
        if (row.status === 'DUPLICATE') {
          ...
          if (duplicateStrategy === 'SKIP') {
            ...
            const existing = await prisma.contact.findUnique({ where: { id: row.existingMatchId } });
            if (existing) {
              await prisma.contact.update({
                where: { id: row.existingMatchId },
                data: {
    ...
    // Send email summary
    sendImportSummaryEmail(req.user!.email, {
